...
module.exports = class Server extends EE{
    ...
    unmarshall(rawData) {
        ...
        let required = ['params', 'method', 'name']

        let missingParams = required.filter( p => !u[p] )

        logger.info("Missing properites: ", missingParams)

        if(missingParams.length > 0) {
            throw new Error("Missing '" + missingParams.join("','") + "' from the propreties of the message")
        }
    ...
    parseMsg(req, res) {
        ...
        if(url.toLowerCase() == config.get("wsp.message_endpoint")) {
            ...
            req.on('data', chunk => {
                ...
                let eventKey = unmarshalled.name + "_" + unmarshalled.method
                logger.info("Emitting event: %s", eventKey)
                this.emit(eventKey, {res, ...unmarshalled.params})
            })
        } else { 
